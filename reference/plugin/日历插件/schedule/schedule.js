	...
	function Schedule (opt) {
		...
		var render = function () {
			...
			for(var i = 0; i < total; i++){
				if(i<startWeek){
					eleTemp.push('<li class="other-html"><span class="dayStyle">'+(lastMonthDay-startWeek+1+i)+'</span></li>')
				}else if(i<(startWeek+fullDay)){
					var nowDate = formartDate(year,month+1,(i+1-startWeek),'-');
					var addClass = '';
					selectedDate == nowDate && (addClass = 'selected-style');
					formartDate(currentYear,currentMonth+1,currentDay,'-') == nowDate && (addClass = 'today-flag');
					eleTemp.push('<li class="current-html" ><span title='+nowDate+' class="currentDate dayStyle '+addClass+'">'+(i+1-startWeek)+'</span></li>')
				}else{
					eleTemp.push('<li class="other-html"><span class="dayStyle">'+(i+1-(startWeek+fullDay))+'</span></li>')
				}
			}
